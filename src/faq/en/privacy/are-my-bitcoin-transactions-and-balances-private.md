...
As Bitcoin enables usage of unique addresses for each incoming payment, it provides a significantly higher layer of privacy than Ethereum. That's why StarBase wallet automatically changes a receive address after each payment. Such approach makes it significantly harder for anyone monitoring the blockchain to identify the user's balance and link all past transactions.
...
Apart from using a unique address for each incoming payment, wallets like StarBase also provide some further advanced privacy enhancing features which has to do with how transactions are structured.
